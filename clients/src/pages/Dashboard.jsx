...
const Dashboard = () => {
  ...
  const handleLogout = async () => {
    try {
      const response = await axios.post(
        "http://localhost:6300/auth/api/logout",
        {},
        {
    ...
    const checkSession = async () => {
      try {
        const response = await axios.get(
          "http://localhost:6300/auth/api/dashboard",
          {
            withCredentials: true,
          }
        );
  ...
  useEffect(() => {
    const checkLoggedUser = async () => {
      try {
        const response = await axios.get(
          "http://localhost:6300/auth/api/getLoggedUser",
          {
            withCredentials: true,
          }
        );
  ...
  if (isLoading) {
    return (
      <div className="flex justify-center items-center">
        <ImSpinner9 size="50px" className="animate-spin absolute top-64" />
      </div>
    );
  }
            ...
            <div className="h-full bg-white dark:bg-gray-700">
              <div className="flex items-center justify-start pt-6 ml-8">
                <p className="text-xl font-bold dark:text-white">
                  <img
                    src="../logo-white.svg"
                    className="w-[150px] relative top-2 left-0"
                  />
                </p>
              </div>
              <nav className="mt-6">
